...
export default function StockTwitsTrendingFeed({ searchTerm, sortOrder }: Props) {
  ...
  if (loading) return <p className="text-center text-gray-400">Loading trending messages...</p>;
  if (error) return <p className="text-center text-red-500">Error: {error}</p>;
  if (filteredAndSortedMessages.length === 0) return <p className="text-center text-gray-400">No trending messages found.</p>;
    ...
    <ScrollArea className="mx-auto p-4 rounded-lg border border-gray-700 bg-gray-900">
      <BentoGrid className="gap-6">
        {filteredAndSortedMessages.map((msg) => (
          <Card
            key={msg.id}
            className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 shadow-2xl rounded-2xl border border-gray-700 hover:scale-[1.01] transition-transform duration-300"
          >
                ...
                <CardTitle className="flex items-center gap-2 text-2xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-600">
                  ...
                  {msg.user.official && (
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger>
                          <Badge variant="outline" className="text-blue-400 cursor-default">Official</Badge>
                        </TooltipTrigger>
            ...
            {msg.symbols.length > 0 && (
              <div className="flex flex-wrap gap-3 px-6 pb-4">
                {msg.symbols.map((sym) => (
                  <Button key={sym.symbol} variant="outline" size="sm" asChild className="text-blue-400 hover:bg-blue-900 hover:text-white transition-colors duration-200">
                    <Link href={`https://stocktwits.com/symbol/${sym.symbol}`} target="_blank" rel="noopener noreferrer">
            ...
            {msg.links && msg.links.length > 0 && (
              <div className="px-6 pb-4 space-y-2">
                {msg.links.map((link, i) => (
                  <Link key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-500 hover:underline transition-colors duration-200 text-sm block">
                    {link.title || link.url}
                  </Link>
                ))}
              </div>
            ...
            {msg.entities.media.length > 0 && (
              <div className="px-6 pb-6 flex gap-4 overflow-x-auto scrollbar-hide snap-x snap-mandatory">
                {msg.entities.media.map((media, i) => (
                  <div key={i} className="snap-start flex-shrink-0 w-full md:w-80">
                    <Image
                      src={media.url}
                      alt="media"
                      className="rounded-xl shadow-xl object-cover w-full h-64 md:h-48"
                      loading="lazy"
                    />
                  </div>
                ))}
