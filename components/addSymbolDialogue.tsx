...
export function AddSymbolDialog() {
  const { activeWatchlist, addToWatchlist, searchSymbols, isLoading } = useWatchlist()
...
useEffect(() => {
  ...
  const handleAddSymbol = async (symbol: string) => {
    if (activeWatchlist) {
      ...
      await addToWatchlist(activeWatchlist.id, symbol)
      setSelectedSymbol('')
      setOpen(false)
      setQuery('')
      setResults([])
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-gray-800 border-gray-700 max-w-md">
        <DialogHeader>
          <DialogTitle className="text-white">Add Symbol to Watchlist</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {results.length > 0 && (
            <div className="max-h-60 overflow-y-auto space-y-1">
              <p className="text-gray-400 text-sm font-medium px-2">Search Results</p>
              {results.map((result) => (
                <div
                  key={result.symbol}
          ...
          {query.length < 2 && (
            <div className="space-y-2">
              <p className="text-gray-400 text-sm font-medium px-2">Popular Symbols</p>
