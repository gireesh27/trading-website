...
export function WatchlistSummary() {
  ...
  const summary = useMemo(() => {
    ...
    const items = activeWatchlist.items.filter(
      (item) => typeof item.price === "number" && typeof item.change === "number"
    );

    const totalValue = items.reduce(
      (sum, item) => sum + item.price! * 100,
      0
    ); // Assuming 100 shares each
        ...
        {summary.topGainer && (
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              {summary.topLoser && summary.topLoser.changePercent! < 0 && (
